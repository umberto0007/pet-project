...
import {FilterId} from "#types/entities/categoryFilters";
...
const CategoryContent: React.FC<ChildProps> = ({products, isLoading}) => {
    ...
    // Создаем state для кнопки 'Показать товары'
    const [applyButtonFilter, setApplyButtonFilter] = useState<FilterId | null>(null)

    // Во избежание рассинхронизации в stateFilter.priceRange по причине игнора priceRange
    // принято решение перевести фильтрацию с авторежима на ручной с общей кнопкой 'Показать товары',
    // для этого создаем draft состояние, в которое помещаем stateFilter.
    const [draftFilterState, setDraftFilterState] = useState(stateFilter)

    const {filteredProducts} = filterProducts(products ?? [], stateFilter)
    ...
    /*console.log(stateFilter.priceRange)*/
                            ...
                            <div className='relative'>
                                ...
                                <ul className={`${!filterVisibilityState.isVisibilityAvailability ? 'max-h-0 overflow-hidden' : 'max-h-screen pt-3'} transition-max-height duration-300 ease-in-out`}>
                                    <AvailabilityFilter
                                        ...
                                        applyButtonFilter={applyButtonFilter}
                                        setApplyButtonFilter={setApplyButtonFilter}
                                    />
                                </ul>
                            </div>
                        </div>
